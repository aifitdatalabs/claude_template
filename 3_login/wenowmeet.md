...
const getStorage = () => {
  ...
  // 네이티브 환경에서는 AsyncStorage 동적 import
...
| `types/` | 공유 타입 정의 |
| `features/auth/` | 인증 도메인 로직 + 컨테이너 + 공통 스타일 |
...
## 10. 웹 디버깅 환경 설정 (react-native-web)

웹 환경에서 실행 시 추가 설정이 필요합니다.

### 문제 1: AsyncStorage 웹 미지원
**에러**: `Uncaught ReferenceError: require is not defined`

**원인**: `@react-native-async-storage/async-storage`는 네이티브 전용이며, 웹에서 `require()` 구문이 동작하지 않음

**해결**: Platform.OS로 분기하여 웹에서는 localStorage 사용 (위 supabase.ts 참조)

---

### 문제 2: process.env 미정의
**에러**: `Uncaught ReferenceError: process is not defined`

**원인**: Webpack은 기본적으로 `process.env`를 정의하지 않음
